...
* customer solution leveraging, avialable platform tools (for example,
...
Once your metrics have identified that there is a problem, the approach for
investigation will often fall into one of the commong categories.
...
* Approach
  * generate a [Diagnostic report](https://nodejs.org/api/report.html)
...
reproduce issue seen in production.o
...
Typically APMs will be able to help you generated and consume
...
If you rely on an APM, It is also important to ensure you have enough licences so
...
A key consideration is how to make the additional compoenents required for problem determination available.
...
The team has used a few approaches to enerate and extract heap snapshots:

* Approach 1
1. Use the `--heapsnapshot-signal=SIGUSR2` command line option to enable heap dump generation on `SIGUSR2` 
1. Add heap-dump script in package.json to trigger SIGUSR2 with kill
1. use `kubectl exec` to run heap-dump script and then `kubectl cp` to copy out
   the dump (or equivalent in non kubernetes environments)

* Approach 2
1. adding  a hidden rest call to the application to trigger heap dump (only accessible internally)
